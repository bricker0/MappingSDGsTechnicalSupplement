...
If this step does not work for you - please use the data I provide in this tutorial (it is the same boundary file). 
...
As an example - here we download specifically SDG indicator 5.5.1 but you are welcome to download any SDG indicator data, or use the data I provide for you. 

First click in the search box and a new window will pop up - search for the indicator you wish to map. For example: I enter the word "emissions" in the search box and get all the indicators with the word emissions in them. I then toggle "INDICATOR 13.2.2 Total greenhouse gas emissions per year
Total greenhouse gas emissions with LULUCF (Mt CO₂ equivalent)  EN_ATM_GHGT_WLU
Total greenhouse gas emissions without LULUCF (Mt CO₂ equivalent)  EN_ATM_GHGT_WOLU"

Next, at the top of the search box you see that "indicators" are highlighted in blue, I click "countries, areas, or regions" by default "all groupings" are selected. Instead select "countries or areas" it may take a moment to process results. The next pop up window - select only the years you want. I recommend only select one or two years. If you select two years, you may need to do some data cleaning later. For simplicity, pick one year.

Alternatively - or another way of describing these directions - Download the data for SDG indicator 5.5.1 from the [Statistics Division of the UN website](https://unstats.un.org/sdgs/dataportal/database). This database stores the data for all SDG indicators. 

Under *Data Series* click the plus sign next to *Select* to expand *Goal 5→Target 5.5→Indicator 5.5.1*.

Click the box next to *INDICATOR 5.5.1: Proportion of seats held by women*. Or the indicator you wish to map - read carefully and see what you selected -  Then, click "Done".
...
In the example of 5.5.1 - Here, you are downloading the percentage of seats in parliament held by women, not just the number of seats held by women, which supports  ***normalization***. Pay close attention to your indicator if it is normalized or not.
...
>
> Open the file in Excel and have a look. Delete any columns you know you do not want now. See if countries are duplicated. Take a look at the data and think carefully about what you need to make your maps. It is often easier to do data cleaning in Excel. Once you are happy with your data, save as csv file.
